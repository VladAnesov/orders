...
function toggleFlash(show, timeout) {
    ...
    var f = function () {
        if (this.getAttribute('preventhide') || this.getAttribute('type') == 'internal/link') {
            ...
            show ? setStyle(this, {position: 'static', top: 0}) : setStyle(this, {
                    position: 'absolute',
                    top: '-5000px'
                });
        } else {
...
function shortCurrency() {
    ...
    each(geByClass('_short_currency'), function () {
        ...
        if (typeof rubEnabled[ff] === 'undefined') {
            ...
            var div = ce('div', {innerHTML: '<b>' + _short + '</b><b>' + _test + '</b>'}, {
                fontFamily: ff,
                fontSize: '24px'
            });
...
var ajax = {
    ...
    post: function (url, query, options) {
        ...
        var o = extend({
            _captcha: false,
            _box: false
        }, options || {}), q = extend({al: o.frame ? -1 : 1}, query), now = vkNow();
            ...
            switch (code) {
                ...
                case 12:
                    ...
                    o._box = validateMobileBox({
                        acceptCaptcha: (code == 11), onDone: function (sid, key) {
                            vk.nophone = 0;
...
function leftBlockUnpaidGiftsHide(blockId, hash) {
    return showFastBox({title: getLang('left_delete_unpaid_gifts_title')}, getLang('left_delete_unpaid_gifts_text'), getLang('global_delete'), function () {
...
function showFastBox(o, c, yes, onYes, no, onNo) {
    return (new MessageBox(typeof(o) == 'string' ? {title: o} : o))
        .content(c)
...
function lockActionsMenuItem(el) {
    if (
        (el = ge(el)) &&
        hasClass(el, 'ui_actions_menu_item') && !hasClass(el, 'ui_actions_menu_item_lock')
    ) {
...
function showWriteMessageBox(e, id) {
    ...
    var box = showBox('al_im.php', {
        act: 'a_write_box',
        to: id
    }, {stat: ['writebox.js', 'writebox.css', 'wide_dd.css', 'page.css', 'emoji.js', 'notifier.css'], cache: 1}, e);
...
function giftsBox(mid, ev, tab) {
    ...
    return !showBox('al_gifts.php', {act: 'box', tab: tab || 'received', mid: mid}, {
        cache: 1,
...
var gSearch = new (function () {
    ...
    this.load = function () {
        ...
        ajax.post('hints.php', {act: 'a_start_hints'}, {
...
/**
 *
 * Äîïèøèòå äîêó, êòî çíàåò î ýôôåêòå îïöèé, äëÿ êîòîðûõ íå äàíî îïèñàíèå
 *
 * @param el
 * @param opts
 text string - òåêñò äëÿ îòîáðàæåíèÿ
 url - àäðåñ äëÿ çàïðàøèâàíèÿ òåêñòà äëÿ îòîáðàæåíèÿ. ðàáîòàåò âìåñòî text
 params - ïàðàìåòðû äëÿ url?
 center: bool
 shift - ìàññèâ, ñîäåðæàùèé ñìåùåíèÿ x, y(, y ïðè âûêèäûâàíèè âíèç(, x ïðè âûêèäûâàíèè âëåâî)) èëè ôóíêöèÿ, âîçâðàùàþùàÿ òàêîé ìàññèâ
 showdt int - (dt - delay time) çàäåðæêà ïåðåä ïîÿâëåíèåì â ìñ
 hidedt int - çàäåðæêà ïåðåä èñ÷åçíîâåíèåì â ìñ
 forcetodown
 forcetoup
 ajaxdt
 hasover
 ...
 slideX
 className
 dir - 'auto', 'top', 'right', 'bottom', 'left' - íàïðàâëåíèå, â êîòîðîì ïîêàçûâàåòñÿ òîð÷àùèé óãîëîê òóëòèïà
 needLeft: bool - çàñòàâëÿåò òóëòèï "ðàçâåâàòüñÿ" â ëåâîì íàïðàâëåíèè îò òî÷êè ïðèêðåïëåíèÿ
 reverseOffset
 checkLeft
 appendEl
 tip
 ...
 ðàñïîëîæåíèå óãîëêà çàäàâàòü ÷åðåç css. ïî-óìîë÷àíèþ ýòî .tt_default.tt_*:before (òåíü) è .tt_default.tt_*:after (ñàì óãîëîê), ãäå âìåñòî * ìîæåò ñòîÿòü êëàññ íàïðàâëåíèÿ óãîëêà, íàïðèìåð down
...
function reportAd(ad_id) {
    showBox('/reports.php?act=a_report_ad_box', {ad_id: ad_id}, {
        params: {width: 370},
...
function zNav(changed, opts, fin) {
    ...
    if (w) {
        ...
        return !showBox('/al_gifts.php', {
            ...
            link: nav.objLoc[0]
        }, {stat: ['gifts.css', 'ui_controls.js', 'ui_controls.css'], cache: 1}, window.event);
    } else if (z == 'validatebox') {
        return !validateMobileBox({
            closeLink: 1, onDone: function () {
                ...
                if (listId) {
                    ...
                    if (playlistId) {
                        ...
                        options = extend(options, {
                            ...
                            module: module,
                            addParams: {force_no_repeat: 1, show_next: 1, playlist_id: playlistId}
                        });
                    }
                }
...
function showWiki(page, edit, e, opts) {
    ...
    ajax.post('wkview.php', extend({act: 'show', loc: nav.objLoc[0]}, page, opts.ads_params), params);
    return cancelEvent(e);
}
...
ElementTooltip.prototype.updatePosition = function () {
    ...
    function setArrowCenteredPosition(side, shift) {
        var style = {}
        style[side] = Math.ceil(ttelSize[0] / 2)/*tt center*/ - border/*borders*/ - arrowSize - (shift || 0)
        /*shift compensation*/
...
(function () {
    ...
    function isViewEnded(elem, scrollY, winHeight) {
        return (
            elem[STARTED] && !isViewable(elem, PERCENT_END, scrollY, winHeight)
        );
    }
...
function audioSearchPerformer(ref, event) {
    ...
    setTimeout(function () {
        if (isInAudioPage) {
            ...
            nav.change({q: performer, performer: 1}, event, {searchPerformer: true, nav: true, isLayer: page.isLayer()})
        } else {
            nav.go(ref, event)
        }
    }, 50)
...
function audioShowActionTooltip(btn, shift, needDownAndLeft) {
    ...
    var options = {
        text: function () {
            return text;
        }, black: 1, shift: shift ? shift : [7, 4, 0], needLeft: true, forcetodown: needDownAndLeft
    };
